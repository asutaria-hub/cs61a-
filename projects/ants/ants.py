...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        #print(self.back)
        if self.back:
            if not isinstance(destination.entrance, Hive):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            qplace = self.place.exit
            while qplace != None:
                if qplace.ant:
                        if qplace.ant.is_container:
                            if qplace.ant.is_buffed == False:
                                qplace.ant.damage = qplace.ant.damage*2
                                qplace.ant.is_buffed = True
                            if qplace.ant.contained_ant:
                                if qplace.ant.contained_ant.is_buffed == False:   
                                    qplace.ant.contained_ant.damage = qplace.ant.contained_ant.damage*2
                                    qplace.ant.contained_ant.is_buffed = True
                        else:
                            if qplace.ant.is_buffed == False:
                                qplace.ant.damage = qplace.ant.damage*2
                                qplace.ant.is_buffed = True 
