...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while vals is not nil:
            if formals is nil:
                raise SchemeError('too many given values')
            else:
                child.define(formals.first, vals.first)
            vals,formals = vals.rest,formals.rest

        if formals is not nil:
            raise SchemeError('too many given formal parameters')
        return child
...
def make_let_frame(bindings, env):
    ...
    while not bindings_check is nil:
        pair = bindings_check.first
        check_form(pair, 2, 2)
        bindings_check = bindings_check.rest
