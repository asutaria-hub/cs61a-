...
def balanced(m):
    ...
    if is_weight(m):
    	...
    	torL = length(left(m))*total_weight(lef)
    	torR = length(right(m))*total_weight(rig)
    	return torR == torL and balanced(lef) and balanced (rig)
...
def make_fib():
    ...
    current, nex = 0, 1
    def fib():
    	nonlocal current, nex
    	result = current 
    	current, nex = nex, nex + current
    	return result
    return fib
